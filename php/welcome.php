<?php
    ...
    $userEntryCount = 0;
    $stmt = oci_parse($conn, "BEGIN GetUserBasicStats(:user_id, :entry_count); END;");
    oci_bind_by_name($stmt, ":user_id", $userId);
    oci_bind_by_name($stmt, ":entry_count", $userEntryCount, 32);
    oci_execute($stmt);

    $esResults = [];
    $searchQuery = $_GET['es_search'] ?? '';
    ...
    if ($isAdmin && isset($_GET['init_es']) && $esService) {
        ...
        $stmt = oci_parse($conn, "
            SELECT e.eid, e.name, e.content, e.image_url,
                (SELECT LISTAGG(c.name, ', ') WITHIN GROUP (ORDER BY c.name)
                    FROM IsIn i JOIN Categories c ON i.caID = c.caID
                    WHERE i.eid = e.eid) as categories
            FROM Entry e
        ");
        oci_execute($stmt);
        ...
        echo "<script>alert('initialized $count entries to ElasticSearch');</script>";
    }

    // --- Delete Entry (admin only) ---
    if ($isAdmin && isset($_GET['delete_entry'])) {
        if ($esService && $esService->client) {
            $esService->deleteEntry($eid);
        }
        ...
        echo "<script>alert('Deleted Entry successfully');</script>";
    }
    ...
    if (!empty($esResults)) {
        foreach ($esResults as $result) {
            $entries[] = ['EID' => $result['eid'], 'NAME' => $result['name'], 'CONTENT' => $result['content']];
        }
    } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_entry'])) {
        ...
        if (!empty($name) && !empty($content) && !empty($selectedCatsAdd)) {
            ...
            $esService->indexEntry([
                ...
                'name' => $name,
                'content' => $content,
                'categories' => implode(', ', $selectedCatsAdd)
            ]);
...
<div class="es-search">
    ...
    <form method="GET" style="display: flex; gap: 10px;">
        <input type="text" name="es_search" value="<?= htmlspecialchars($searchQuery) ?>" 
               placeholder="Enter keywords..." style="flex: 1; padding: 10px; border: none; border-radius: 5px;">
        <button type="submit" style="background: #ff6b6b; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">
            Search
        </button>
    ...
    <?php if (!empty($searchQuery)): ?>
        <div style="margin-top: 15px;">
            <p>Search for "<strong><?= htmlspecialchars($searchQuery) ?></strong>" - <?= count($esResults) ?> result(s)</p>
            <?php if (empty($esResults)): ?>
                <p style="color: #ff6b6b; font-style: italic;">No results found. Try different keywords or initialize the index.</p>
            <?php else: ?>
                <?php foreach ($esResults as $result): ?>
                    <div class="es-result">
                        <strong><?= htmlspecialchars($result['name']) ?></strong>
                        <span style="float: right; background: #4ecdc4; color: white; padding: 2px 8px; border-radius: 10px; font-size: 12px;">
                            Relevance: <?= number_format($result['score'], 2) ?>
                        </span>
                        <p style="margin: 5px 0; font-size: 14px;"><?= htmlspecialchars(substr($result['content'], 0, 100)) ?>...</p>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
...
<form method="GET">
    ...
    <?php endforeach; ?>
    <?php if (!empty($searchQuery)): ?>
        <input type="hidden" name="es_search" value="<?= htmlspecialchars($searchQuery) ?>">
    <?php endif; ?>
...
<div class="feature-card">
    ...
    <ul>
        ...
        <li><strong>Triggers:</strong> Automatically record entry changes</li>
        <li><strong>ElasticSearch:</strong> Fast searches using inverted indexes</li>
    </ul>
</div>
